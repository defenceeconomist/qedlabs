...
<!-- lab-downloads:start -->
::: {.resource-links}
[Quarto document](downloads/hisp-ie-practice-lab.qmd){download="hisp-ie-practice-lab.qmd"}
[R Jupyter notebook](notebooks/hisp-ie-practice-lab.ipynb){download="hisp-ie-practice-lab.ipynb"}
[Complete lab ZIP](downloads/hisp-ie-practice-lab.zip){download="hisp-ie-practice-lab.zip"}
[Data and setup](data.qmd#hisp-ie-practice-lab)
:::
<!-- lab-downloads:end -->
...
The repository and complete lab ZIP include the original HISP teaching bundle:

- `HISP Book replication.do`
- `evaluation.dta`
- `evaluation.csv`

The CSV, Stata file, and original replication script are available on the [lab data page](data.qmd#hisp). No external data folder or internet connection is needed to run the worksheet.
...
data_dir <- file.path(qed_data_directory(), "hisp")
invisible(qed_data("hisp")) # Verify both the R snapshot and the original files.
evaluation_csv <- file.path(data_dir, "evaluation.csv")

if (!file.exists(evaluation_csv)) {
  stop(
    paste0(
      "The bundled HISP data folder does not contain evaluation.csv: ",
      evaluation_csv
    ),
    call. = FALSE
  )
...
For a dedicated explanation of the four-cell contrast, parallel trends, and log-outcome interpretation, complete the [DiD foundations lab](difference-in-differences-foundations-lab.qmd) first. The HISP branch remains a compact application inside this multi-method replication.
