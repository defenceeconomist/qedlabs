...
[Download the R Jupyter notebook](notebooks/difference-in-differences-foundations-lab.ipynb){.cta-button download="difference-in-differences-foundations-lab.ipynb"}
...
The `injury` data are distributed with the [`wooldridge` R package](https://search.r-project.org/CRAN/refmans/wooldridge/html/injury.html). We use the package copy rather than downloading a separate CSV.

```{r}
required_packages <- c("wooldridge", "dplyr", "ggplot2", "fixest")
...
For a log outcome, $100\widehat{\delta}$ is a convenient small-effect approximation. The exact conversion is $100[\exp(\widehat{\delta})-1]$.
...
State the result as an estimated percentage change in benefit duration for high earners relative to the low-earner counterfactual—not as a change in “logged weeks.”
