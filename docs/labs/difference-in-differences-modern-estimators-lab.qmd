...
county_audit <- mpdta |>
  ...
  summarise(
    ...
    treatment_reversal = any(diff(treat) < 0),
    .groups = "drop"
  )
...
The relative-magnitudes restriction indexes how large post-treatment deviations from parallel trends may be relative to the largest observed pre-treatment deviation.
...
The sensitivity interval for this cohort already includes zero at the first reported restriction value. The exercise is still informative: sensitivity analysis should report how conclusions change under stated violations, not manufacture a decisive result.
