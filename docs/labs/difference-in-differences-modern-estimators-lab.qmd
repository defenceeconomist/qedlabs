...
aliases:
  - /labs/difference-in-differences-modern-estimators-python-lab.html
format:
  html: default
execute:
  echo: true
...
This is the third difference-in-differences lab. The guided core takes about 45–60 minutes; a lightweight calibrated-bias stress test is an optional extension.

<!-- lab-downloads:start -->
::: {.resource-links}
[Quarto document](downloads/difference-in-differences-modern-estimators-lab.qmd){download="difference-in-differences-modern-estimators-lab.qmd"}
[R Jupyter notebook](notebooks/difference-in-differences-modern-estimators-lab.ipynb){download="difference-in-differences-modern-estimators-lab.ipynb"}
[Complete lab ZIP](downloads/difference-in-differences-modern-estimators-lab.zip){download="difference-in-differences-modern-estimators-lab.zip"}
[Data and setup](data.qmd#difference-in-differences-modern-estimators-lab)
:::
<!-- lab-downloads:end -->
...
The main workflow implements Callaway and Sant'Anna with the [`did` package](https://bcallaway11.github.io/did/) and compares it with Sun–Abraham in `fixest` [@callaway2021multipletime; @sunabraham2021dynamic]. The extension demonstrates the logic of calibrating possible post-treatment bias to observed pre-treatment discrepancies without introducing a conic-optimisation dependency. It is a teaching stress test, not an implementation of the formal Rambachan–Roth procedure [@rambachanroth2023parallel]. See the [Callaway–Sant'Anna notes](../notes/did/callaway-santanna-multiple-periods-notes.qmd) and [modern DiD synthesis](../notes/did/roth-et-al-did-synthesis-notes.qmd).
...
## Optional Extension: A Calibrated-Bias Stress Test

To keep the exercise transparent, restrict the data to the 2006 cohort and never-treated counties. This creates a non-staggered design with two estimated pre-treatment coefficients, an omitted 2005 reference period, and two post-treatment coefficients.
...
The following screen asks how the first post-treatment confidence interval changes if an unknown bias may be as large as a multiple of the largest observed pre-treatment discrepancy. It deliberately uses only arithmetic and the fitted covariance matrix. Unlike formal Rambachan–Roth sensitivity intervals, it treats the observed pre-period scale as fixed and should therefore be reported as a teaching diagnostic rather than a confidence set with uniform coverage.

```{r}
pre_index <- term_year < 2005
first_post_index <- which(term_year == 2006)
pre_scale <- max(abs(event_coefficients[pre_index]))
post_estimate <- unname(event_coefficients[first_post_index])
post_standard_error <- sqrt(event_covariance[first_post_index, first_post_index])

sensitivity_results <- tibble(
  bias_multiple = seq(0, 2, by = 0.5),
  estimate = post_estimate,
  bias_allowance = bias_multiple * pre_scale,
  lower = post_estimate - 1.96 * post_standard_error - bias_allowance,
  upper = post_estimate + 1.96 * post_standard_error + bias_allowance
)
...
first_zero_including_multiple <- if (any(zero_included)) {
  min(sensitivity_results$bias_multiple[zero_included])
} else {
  NA_real_
}

sensitivity_results
data.frame(first_bias_multiple_including_zero = first_zero_including_multiple)

stopifnot(
  nrow(sensitivity_results) == 5L,
  all(sensitivity_results$lower <= sensitivity_results$upper),
  all(is.finite(sensitivity_results$bias_multiple))
)
```

```{r, fig.height=5.2, fig.width=9}
ggplot(sensitivity_results, aes(bias_multiple, estimate)) +
  geom_hline(yintercept = 0, colour = "gray55") +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2) +
  labs(
    x = "Allowed bias as a multiple of the largest observed pre-period discrepancy",
    y = "First post-treatment estimate and expanded interval",
    title = "A transparent sensitivity screen"
  ) +
  theme_minimal(base_size = 12)
```

The target is the first post-treatment effect (2006), not the average of both post-treatment periods. The exercise is informative only when the calibration and its limitations are stated: it shows how quickly a conclusion changes under a simple bias allowance, not a formal robustness guarantee.
...
- The sensitivity exercise concerns the 2006 cohort and first post-period only. Report it as a calibrated-bias screen, not as a formal Rambachan–Roth confidence set.
